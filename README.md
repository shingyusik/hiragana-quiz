...
히라가나를 암기하기 위한 정적 웹 퀴즈 페이지입니다.  
랜덤으로 표시되는 히라가나를 보고 키보드로 발음을 입력하면 정답 여부를 확인할 수 있습니다.
...
1. `테스트 시작`을 누릅니다.
2. 화면에 보이는 히라가나의 발음을 입력합니다.
3. 알고 있으면 `정답 확인`, 모르겠으면 `정답 보기`를 누릅니다.
4. 결과를 확인한 뒤 `다음 문제`로 넘어갑니다.
5. 모든 문제를 풀면 `다시 섞기`로 새로 시작할 수 있습니다.
...
문제 데이터와 허용 답안은 [script.js](/Users/shinggyusik/Desktop/01_Project/hiragana-quiz/script.js) 상단의 `HIRAGANA_SET` 배열에서 수정할 수 있습니다.
